...
import { Play, Loader2, Check, Clock } from "lucide-react";
...
export default function TaskCard({ task, onClaim }: Props) {
  ...
  const canClaim = !isAdTask || viewedAt === null || waitRemaining <= 0.05;
  ...
  async function handleClick() {
    ...
    // Ad tasks: enforce 10s dwell
    if (isAdTask && adUrl) {
      if (viewedAt === null) {
        // first click — open ad, start timer, don't claim yet
      ...
      // Offer tasks: still open link if present, but no 10s gate
      const webApp = (window as any).Telegram?.WebApp;
      if (webApp?.openLink) {
        try { webApp.openLink(adUrl); } catch { window.open(adUrl, "_blank"); }
      } else {
    ...
    <div className="relative rounded-card bg-surface border border-border p-4">
      ...
      {isAdTask && viewedAt !== null && waitRemaining > 0 && task.status === "available" && (
        <div className="mt-3 text-brand text-xs flex items-center gap-1.5 bg-brand/10 border border-brand/20 rounded-lg px-3 py-2">
          <Clock size={12} /> Ad opened — stay {Math.ceil(waitRemaining)}s more, then tap Claim
        </div>
      )}
